#!/usr/bin/env python3
"""
Validate the FP8 (e4m3) emulation for evo2_1b_base on Strix Halo.
...
def evaluate(evo, seqs, max_len: int | None) -> list[tuple[float, float]]:
    ...
    for seq in seqs:
        ...
        with torch.no_grad():
            # StripedHyena.forward returns (logits, inference_params); Evo2.__call__
            # wraps that tuple without unpacking, so call .model.forward directly.
            fwd = evo.model.forward(ids)

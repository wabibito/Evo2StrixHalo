...
PATCH_MARK = "# Evo2StrixHalo patch"
...
def patch_engine(root: Path) -> int:
    ...
    edits = [
        (
            ...
            (
                'with torch.autocast(  # Evo2StrixHalo patch: autocast\n'
        ...
        (
            ...
            (
                "state_S = torch.fft.fft(state_s, n=fft_size)  # Evo2StrixHalo patch: fft_repeat\n"
...
def patch_engine_empty_cache(root: Path) -> int:
    ...
    edits = [
        (
            ...
            (
                "(torch.cuda.empty_cache() if torch.cuda.is_available()  # Evo2StrixHalo patch: empty_cache\n"
...
# --- Evo2StrixHalo patch: rotary_torch_fallback ----------------------------------
    ...
    edits = [
        (
            ...
            (
                ...
                "and qkv.is_cuda:  # Evo2StrixHalo patch: qkv_view_path"
            ),
            "qkv_view_path",
        ),
    ]
...
def patch_rotary_torch_fallback(root: Path) -> int:
    ...
    text, n_import = re.subn(
        ...
        (
            "try:  # Evo2StrixHalo patch: rotary_torch_fallback\n"
...
def patch_attn_interface_optional(root: Path) -> int:
    ...
    edits = [
        (
            ...
            (
                "try:  # Evo2StrixHalo patch: optional_flash_attn\n"
...
def patch_cuda_device_contexts(root: Path) -> int:
    ...
    for p in targets:
        ...
        if "import contextlib" not in text:
            ...
            text, _ = re.subn(
                r"(import torch[^\n]*\n)",
                r"\1import contextlib  # Evo2StrixHalo patch: contextlib\n",
                text,
                count=1,
            )
...
def _detect_backend() -> str:
    """Return 'rocm', 'mps', or 'cpu' to decide which patch subset to apply."""
    try:
        import torch
...
def main() -> int:
    ...
    ap.add_argument("--backend", choices=["auto", "rocm", "mps"], default="auto",
                    help="which patch subset to apply (default: auto-detect)")
    args = ap.parse_args()
    ...
    total = 0
    # The flash-attn fallbacks are needed on BOTH ROCm (gfx1151 has no
    # flash-attn) and MPS. They make vortex run with torch SDPA instead.
    ...
    if backend == "mps":
        # MPS-only fixes: MPS is not a 'cuda' device, so autocast("cuda"),
        # .cuda() contexts, empty_cache and mem-query calls must be rerouted.
        # On ROCm these all work natively (HIP maps the CUDA API), so they are
    ...
    print(f"\napplied {total} edit(s) for backend '{backend}'.")
    if backend == "rocm":
        print("Note: ROCm support is new; if a forward pass fails on a CUDA-API "
              "call, re-run with --backend mps to apply the broader fallback set.")
    return 0
